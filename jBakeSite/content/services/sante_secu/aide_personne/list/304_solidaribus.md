title=Solidaribus
date=2026-07-17
type=org_openCiLife_block
category=aide_personne
subTemplate=sslf.servicesSubTemplate
tags=
status=published
contentImage=<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="darkGreen" aria-hidden="true" data-tsd-source="/src/routes/services.index.tsx:239:11"><path d="M8 6v6"></path><path d="M15 6v6"></path><path d="M2 12h19.6"></path><path d="M18 18h3s.5-1.7.8-2.8c.1-.4.2-.8.2-1.2 0-.4-.1-.8-.2-1.2l-1.4-5C20.1 6.8 19.1 6 18 6H4a2 2 0 0 0-2 2v10h3"></path><circle cx="7" cy="18" r="2"></circle><path d="M9 18h5"></path><circle cx="16" cy="18" r="2"></circle></svg>
exerpt=Accueil social et aides
anchorId=solidaribus

location=parking place du monument aux morts
phone=05 55 04 20 00
email=solidaribus.spf87@orange.fr
freeDate=1fois par mois
order=304
~~~~~~
Toute personne rencontrant des difficultés sociales pourra se présenter à la permanence, munie de ses justificatifs de situation, pour bénéficier de l'aide du Secours Populaire.

![croix rouge deventure](${webleger.build.host}/images/services/solidaribus_infos.jpg)

> **Adresse**
> parking place du monument aux morts 87160 Saint-Sulpice-les-Feuilles

> **Horaires**
> parking place du monument aux morts

- accueil social
- aide alimentaire
- accès aux vacances
- aides aux démarches en ligne
- colis de Noël
- information et orientation.